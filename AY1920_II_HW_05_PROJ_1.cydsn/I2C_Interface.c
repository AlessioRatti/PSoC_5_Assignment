...
ErrorCode I2C_Peripheral_ReadRegisterMulti(uint8_t device_address,
                                           ...
                                           uint8_t* data)
{
    // Send start condition (W)
    uint8_t error = I2C_Master_MasterSendStart(device_address,I2C_Master_WRITE_XFER_MODE);
    if (error == I2C_Master_MSTR_NO_ERROR)
    {
        // Write address of register to be read with the MSB equal to 1 (autoincrement)
        error = I2C_Master_MasterWriteByte(register_address|0x80);
        if (error == I2C_Master_MSTR_NO_ERROR)
        {
            // Send restart condition (R)
            error = I2C_Master_MasterSendRestart(device_address, I2C_Master_READ_XFER_MODE);
            if (error == I2C_Master_MSTR_NO_ERROR)
            {
                // Continue reading until we have register to read
                for(uint8_t pBuffPtr = 0 ; pBuffPtr<register_count-1; pBuffPtr++)
                {
                    // Read byte with acknowledgement
                    *(data+pBuffPtr) = I2C_Master_MasterReadByte(I2C_Master_ACK_DATA);
                }
                                        ...
                                        uint8_t* data)
{
    ...
    if (error == I2C_Master_MSTR_NO_ERROR)
    {
        ...
        if (error == I2C_Master_MSTR_NO_ERROR)
        {
            ...
            for(uint8_t pBuffPtr = 0 ;
                pBuffPtr<register_count && error == I2C_Master_MSTR_NO_ERROR;
                pBuffPtr++)
            {
                // Write byte
                error = I2C_Master_MasterWriteByte(*(data+pBuffPtr));
            }
        }
    }
    // Sends immediate stop condition in case something didn't work out correctly
    I2C_Master_MasterSendStop();
    // Return error code
    return error ? ERROR : NO_ERROR;
}
